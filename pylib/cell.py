...
class Cell(pd.Series):
    ...
    def __init__(self, incell, *, name: str='', **kwargs):
        """ incell: Expect the input cell to be dict-like
            name: The name to assign to the cell.
        """
        super().__init__(incell, **kwargs)
    ...
    def random(self, *, alpha=None, S=None, name='random', random_state=None):
        """ Return a randomized version of the input cell with a new set of weights.
        ...
        S = S or self.S
        mu = self.expected_n(alpha) * S/self.S
        size = stats.poisson.rvs(mu, random_state=random_state)
        newcell = dict(
            n=size,  
            S=S, 
            B=self.B*S/self.S,
            w=self.weight_sampler()(stats.uniform.rvs(scale=1, size=size, random_state=random_state)
                                      )
        )
    ...
    def generate_sim_cells(self, N=10000, S=None, alpha=None, ):
        ...
        def make_sim_cell(i):
            return self.random(S=S,alpha=alpha, random_state=i, name=f"sim_{i}")
